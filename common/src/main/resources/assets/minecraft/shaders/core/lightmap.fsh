...
void main() {
    ...
    float brightness = lightmapInfo.BrightnessFactor;
    float min_level = 0.0;
    
    // Check if Halfbright is active (encoded in negative BrightnessFactor)
    if (brightness < -0.5) {
        float encoded = -brightness - 1.0;
        brightness = floor(encoded / 1000.0) / 100.0;
        min_level = mod(encoded, 1000.0) / 150.0;
        
        // Scale the lower light levels up smoothly, keeping exactly 16 steps
        block_level = mix(min_level, 1.0, block_level);
        sky_level = mix(min_level, 1.0, sky_level);
    }
    ...
    color = mix(color, notGamma, brightness);
